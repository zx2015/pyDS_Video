...
from ds_video.api import ApiError, FileEntry, FileStationClient
...
class MainWindow(QMainWindow):
    ...
    def _load_shares(self) -> None:
        try:
            ...
            QMessageBox.critical(self, "加载失败", f"无法获取共享文件夹列表：{exc}")
        ...
        if not video_shares:
            self._status_hint.showMessage(
                "未找到名为 video/电影 的共享文件夹，已显示全部共享文件夹", 6000
            )
            video_shares = shares
        ...
        for share in video_shares:
            try:
                ...
                QMessageBox.critical(self, "加载失败", f"无法获取目录内容：{exc}")
                continue
    ...
    def _populate_subfolders(self, parent_item: QTreeWidgetItem, path: str) -> None:
        ...
        for entry in entries:
            ...
            child.setData(0, self._PATH_ROLE, entry.path)
    ...
    def _load_file_list(self, path: str) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self, "加载失败", f"无法获取文件列表：{exc}")
